...
stripExpirations <- function(chain, exp=NULL){
	if (doDebug) print("stripExpirations")
	if (is.null(exp))
		exp <- chain[1,]$expiry
...
getStrikes <- function(chain){
	...
	strikes <- data.frame(min=min, max=max, upper=upper, lower=lower, interval=interval)
	return(strikes)
}
shinyServer(function(input, output, clientData, session){
	...
	lower <- reactive({input$slider[1]})
	upper <- reactive({input$slider[2]})
...
#		sliderInput("slider", "Strikes", value=c(strikes()$lower,strikes()#$upper), min=strikes()$min, max=strikes()$max)	})
	...
	googChains <- reactive({getOptionChainGoogle(input$ticker)})
	 chains <- reactive({stripGoogleChain(googChains())})
	chain <- reactive({stripExpirations(chains())})
...
p <- p + geom_area(aes(y = putOI,  fill = "1 put", colour = "1 put",   stat = "bin"), alpha = 0.5) + geom_area(aes(y = callOI, fill = "2 call",colour = "2 call",  stat = "bin"), alpha = 0.5)
	...
	p <- p + theme(legend.title = element_blank()) + theme(axis.text.x=element_text(angle=90,size=theSize))+ theme(panel.grid.minor.y = element_blank(), panel.grid.major.y = element_blank())+ theme(legend.justification=c(1,1), legend.position=c(1,1))+ ylab("open interest")
	p
	})
    })   
